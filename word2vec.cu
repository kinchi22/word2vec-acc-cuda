...
int binary = 0, cbow = 1, debug_mode = 2, window = 5, min_count = 5, num_streams = 1, min_reduce = 1;
int *vocab_hash;
...
void TrainModelThread()
{
	...
	// use in kernel
	int total_sent_len, streamIdx = 0, blockSize = 32;
...
int main(int argc, char **argv) {
	...
	if (argc == 1) {
		...
		printf("\t-streams <int>\n");
		printf("\t\tUse <int> CUDA streams (default 1)\n");
		printf("\t-iter <int>\n");
	...
	if ((i = ArgPos((char *)"-streams", argc, argv)) > 0) num_streams = atoi(argv[i + 1]);
